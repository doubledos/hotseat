...
function opposingTeam(t){ return (t||state.hotSeatTeam)==='A'?'B':'A'; }
function hotSeatPlayer(){ return playerById(state.hotSeatPlayerId); }
// Level currently in play for a team. Classic mode shares one ladder; race mode gives each team its own.
function activeLevel(team){
  if(state.gameMode==='race') return state.race[team||state.hotSeatTeam].current;
  return state.ladderCurrent;
}
function isLevelWon(team, l){
  if(state.gameMode==='race') return !!state.race[team||state.hotSeatTeam].won[l-1];
  return !!state.ladderWon[l-1];
}
function levelMoney(l){ return LEVEL_MONEY[(l||activeLevel())-1]||100; }
function levelType(l){ const idx=(l||activeLevel())-1; return state.levelTypes[idx]==='puzzle'?'puzzle':'trivia'; }
function levelDiff(l){ const idx=(l||activeLevel())-1; if(state.levelTypes[idx]==='puzzle') return 'puzzle'; return LEVEL_DIFFICULTY_DEFAULT(l||activeLevel()); }
...
async function winLevel(team, amount){
...
function correctDisplayIdx(q){ return q&&q.displayOrder?q.displayOrder.indexOf(0):-1; }
...
function makeCurrentQuestion(q, level){
...
function wheelMoneyLabel(n){
  if(n>=1000000) return '$'+(n/1000000).toFixed(n%1000000===0?0:1)+'M';
  if(n>=1000) return '$'+Math.round(n/1000)+'K';
  return money(n);
}
function buildBombWheelOutcomes(){
...
function wheelOutcomeResultText(o){
  if(!o) return '';
  if(o.type==='win') return money(o.amount);
...
const WHEEL_FIJI_IMG='https://upload.wikimedia.org/wikipedia/commons/3/3f/Sunset_with_coconut_palm_tree%2C_Fiji.jpg';
function wheelZoomVisual(o){
...
function showWheelZoom(idSuffix, outcome, pulse){
...
function buildWheelHTML(wheel, idSuffix, opts){
  opts=opts||{};
...
function revealWheelOutcome(outcome, wheel, resultEl, isTV){
...
function onWheelLanded(wheel, idSuffix, resultEl){
...
function animateWheel(wheel, idSuffix){
